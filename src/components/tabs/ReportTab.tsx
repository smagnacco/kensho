  ...
  if (generations.length === 0) {
    return (
      <div className="p-4">
        ...
        >
          El reporte aparecera al finalizar el experimento.
        </div>
      </div>
    )
  }
    ...
    <div className="p-4 space-y-8">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
          <Card label="Modelo A" value={config.agentA.model} color="#00e5cc" />
          <Card label="Modelo B" value={config.agentB.model} color="#a78bfa" />
          <Card label="Modelo P" value={config.agentP.model} color="#f59e0b" />
          <Card label="Generaciones" value={generations.length} />
          <Card label="Interacciones" value={allInteractions.length} />
          <Card label="Emergidos" value={emerged.length} color="#a78bfa" />
          <Card label="Costo total" value={`$${totalCost.toFixed(4)}`} color="#f59e0b" />
        </div>
      </section>

      {/* 2. Coherencia evolution */}
      <section>
        <h2 className="text-sm font-semibold mb-3" style={{ color: '#e5e7eb' }}>
      ...
      {emerged.length > 0 && (
        <section>
          <h2 className="text-sm font-semibold mb-3" style={{ color: '#e5e7eb' }}>
            Conceptos emergentes (evolucionados)
          </h2>
          <div className="space-y-3">
            {emerged.map((i, idx) => (
              <div
                ...
                style={{ background: '#0e0e1c', border: '1px solid #a78bfa44' }}
              >
                ...
                <p className="text-xs leading-relaxed mb-1" style={{ color: '#9ca3af' }}>
                  {i.definition}
                </p>
                <p className="text-xs" style={{ color: '#6b7280' }}>
                  Tension: {i.tensionInsight}
                </p>
              </div>
            ))}
          </div>
      ...
      {/* 3b. Perturbaciones entre generaciones */}
            ...
            {perturbations.map((p, idx) => (
              ...
              >
                ...
                <div className="text-xs space-y-1">
                  <div>
                    <span style={{ color: '#00e5cc' }}>Agent-A recibió: </span>
                    <span style={{ color: '#9ca3af' }}>{p.perturbationA}</span>
                  </div>
                  <div>
                    <span style={{ color: '#a78bfa' }}>Agent-B recibió: </span>
          ...
          <div className="space-y-2">
            {accepted.map((i, idx) => (
              <div
                key={idx}
                className="rounded p-3"
                style={{ background: '#0e0e1c', border: '1px solid #10b98144' }}
              >
                <div className="font-mono text-sm font-medium mb-1" style={{ color: '#10b981' }}>
                  {i.concept}
                </div>
      ...
      {/* 5. World Models finales */}
      <section>
        <h2 className="text-sm font-semibold mb-3" style={{ color: '#e5e7eb' }}>
          World Models finales
        </h2>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {[
            { label: 'Agent-A', color: '#00e5cc', wm: lastA },
            { label: 'Agent-B', color: '#a78bfa', wm: lastB },
          ].map(({ label, color, wm }) => (
            <div
              key={label}
          ...
          <table className="w-full text-xs font-mono">
            <thead>
              <tr style={{ background: '#0e0e1c', borderBottom: '1px solid #1a1a2e' }}>
                {['Gen', 'Ronda', 'Concepto', 'Generacion', 'Critica', 'Perturbación', 'Subtotal'].map((h) => (
            ...
            <tbody>
                ...
                return g.interactions.map((i, idx) => {
                  ...
                  return (
                    <tr
                      ...
                      style={{
                        background: rowIdx % 2 === 0 ? '#09090f' : '#0e0e1c',
                        borderBottom: '1px solid #1a1a2e',
                      }}
                    >
                      <td className="px-3 py-1.5" style={{ color: '#9ca3af' }}>
                        {i.generation}
                      </td>
                      <td className="px-3 py-1.5" style={{ color: '#9ca3af' }}>
                        {i.round}
                      </td>
                      <td className="px-3 py-1.5 max-w-[140px] truncate" style={{ color: '#e5e7eb' }}>
                        {i.concept}
                      </td>
                      <td className="px-3 py-1.5" style={{ color: '#f59e0b' }}>
                        ${i.stageCosts.generation.toFixed(5)}
                      </td>
                      <td className="px-3 py-1.5" style={{ color: '#f59e0b' }}>
                        ${i.stageCosts.critique.toFixed(5)}
                      </td>
              ...
              <tr style={{ background: '#0e0e1c', borderTop: '2px solid #1a1a2e' }}>
                <td colSpan={6} className="px-3 py-2 font-semibold" style={{ color: '#e5e7eb' }}>
                  TOTAL
                </td>
